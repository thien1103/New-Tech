...
const studentController = {
  ...
  getProfileStudents: async (req, res) => {
    try {
      ...
      // Sử dụng Mongoose để tìm student theo studentId
      ...
      // Trả về thông tin hồ sơ student nếu tìm thấy
      res.json({ student });
    } catch (error) {
      console.error(error);
      res.status(500).json({ error: "Internal Server Error" });
  ...
  updateProfileStudent: async (req, res) => {
    const studentId = req.params.studentId;
    const updateData = req.body; // Giả sử dữ liệu cập nhật được gửi trong phần thân của yêu cầu
    ...
    try {
      ...
      // Lưu tài liệu sinh viên đã được cập nhật
      await student.save();
  ...
  availabledissertations: async (req, res) => {
    try {
      ...
      const availableDissertations = await Dissertation.find({
      ...
      res
        .status(500)
        .json({ error: "Error retrieving available dissertations." });
    }
  },
  ...
  registerDissertation: async (req, res) => {
    ...
    try {
      ...
      if (!student || !dissertation) {
        return res
          .status(404)
          .json({ error: "Sinh viên hoặc đề tài không tồn tại." });
      }
      ...
      if (isRegistered) {
        return res
          .status(400)
          .json({ error: "Sinh viên đã đăng ký đề tài này." });
      }
      ...
      res
        .status(200)
        .json({ success: "Đăng ký đề tài thành công. Chờ xét duyệt" });
    } catch (error) {
      console.error(error);
      res.status(500).json({ error: "Đã xảy ra lỗi trong quá trình xử lý." });
    }
